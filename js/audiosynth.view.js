function AudioSynthView() {
	...
	var fnPlayKeyboard = function (e) {
		...
		//
		var notePressed = keyboard[e.keyCode].split(",");
		notePressed = notePressed[0] + (4 + parseInt(notePressed[1]));
		notesPlayed.push(notePressed);
		if (notesPlayed.length > 20) {
			notesPlayed.shift();
		}
		...
		if (currPage == "Play") {
			...
			let boxij = "box" + boxIndex + tryNumber;
			boxIndex++;
			document.getElementById(boxij).innerText = notePressed;
			if (boxIndex > gamePitches.length - 1) {
				colorBoxes();
				if (!gameWon && tryNumber <= maxTries - 1) {
					//createBoxes();
					boxIndex = 0;
					tryNumber++;
   ...
   //         }
		...
		//document.getElementById('kbNotesPlayed').innerHTML += "," + notePressed[0] + (4 + parseInt(notePressed[1]));
		//
