...
if(useModifiedCode) {
	        for (var i = this._events.length - 1; i >= 0; i--) {
	            ...
	            if (event instanceof Tone.Part) {
	                ...
	                if(event.length === 0) {
	                    console.log('event.length === 0 early return of Part.remove')
	                    this._events.splice(i, 1);
	                    return this;
	                }
					if(event instanceof Tone.Sequence) {
						for(var j = event.length -1; j >= 0; j--) {
    	                    if(event.at(j) === null) { // oddly sub-sequences show as null
//    	                        if(subArrayOffset==0) {
    	                            subArrayOffset = time;
//    	                            console.log('    | null or Sub-Sequence >>>>>>>>>>>>>> -- (typeof(event.at(j='+j+')) ='+typeof(event.at(j)) +' event.at(j='+j+')='+event.at(j) );
    	                            event.remove(j,value);
//    	                        }
    	                    }
//    	                    console.log('  | Sequence -- (event instanceof Tone.Sequence === true) CALLING: event.remove(j='+j+',value='+event.value+')');
                            event.remove(j, value);
    	                    subArrayOffset = 0;
						}
					} else { // Tone.Part
...
//----------------- MS new code ---------------------------
	                if (event.startOffset-subArrayOffset === time) {
//--------------------------------------------------------------------
...
// end original Tone.js r11 - for loop
}
	        return this;
	    };
	    /**
